...
@Test func isCheck() {
    let checkPosition = FenSerialization.default.deserialize(
        fen: "3k4/8/8/8/5q2/8/8/5K2 w - - 0 1")
    #expect(
        StandardRules().isCheck(in: checkPosition) == true,
        "Position: \(FenSerialization.default.serialize(position: checkPosition))")

    let notCheckPosition = FenSerialization.default.deserialize(
        fen: "3k4/8/8/8/8/4q3/8/5K2 w - - 0 1")
    #expect(
        StandardRules().isCheck(in: notCheckPosition) == false,
        "Position: \(FenSerialization.default.serialize(position: notCheckPosition))")
}

@Test func isMate() {
    let matePosition = FenSerialization.default.deserialize(
        fen: "3k3R/8/3K4/8/8/8/8/8 b - - 0 1")
    #expect(
        StandardRules().isMate(in: matePosition) == true,
        "Position: \(FenSerialization.default.serialize(position: matePosition))")
    ...
    let stalematePosition = FenSerialization.default.deserialize(
        fen: "8/8/8/8/8/6k1/5q2/7K w - - 0 1")
    #expect(
        StandardRules().isMate(in: stalematePosition) == false,
        "Position: \(FenSerialization.default.serialize(position: stalematePosition))")
}
